...
// TestSecurityHeadersMiddleware_APINoStore verifies that /api/ responses default to
// Cache-Control: no-store so a CDN never serves one user's session or config data to
// another, while explicit handler policies and non-API paths are left alone.
func TestSecurityHeadersMiddleware_APINoStore(t *testing.T) {
	tests := []struct {
		...
		{name: "api path defaults to no-store", path: "/api/session/is-auth-enabled", want: "no-store"},
		{name: "api path keeps handler policy", path: "/api/branding/logo", handlerSets: "private, max-age=86400", want: "private, max-age=86400"},
		{name: "api path keeps streaming no-cache", path: "/api/logs/stream", handlerSets: "no-cache", want: "no-cache"},
